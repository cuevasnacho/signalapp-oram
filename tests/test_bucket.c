...
int test_bucket_store_put_get_jazz()
{
    bucket_store *store0 = bucket_store_create(11);
    ...
    u64 bucket_id = 1234;
    bucket_store_write_bucket_blocks_jazz(store0, bucket_id, blocks);

    u8 bucket_data0[DECRYPTED_BUCKET_SIZE];
    block* new_blocks0 = (block*)bucket_data0;
    bucket_store_read_bucket_blocks_jazz(store0, bucket_id, new_blocks0);
    assert(new_blocks0[0].id == block1.id);
    assert(new_blocks0[1].id == block2.id);
    for (size_t i = 2; i < BLOCKS_PER_BUCKET; ++i)
    {
...
int test_bucket_store_clear_jazz()
{
...
void public_bucket_store_tests()
{
    ...
    RUN_TEST(test_bucket_store_put_get_jazz());
    RUN_TEST(test_bucket_store_clear());
    RUN_TEST(test_bucket_store_clear_jazz());
}
